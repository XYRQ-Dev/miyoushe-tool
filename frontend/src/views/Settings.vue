  ...
  <div class="app-page settings-page">
      ...
      <el-form label-width="120px" :model="taskForm">
        ...
        <el-form-item label="Cron 表达式">
          <el-input
            ...
            style="width: 200px"
          />
          <span class="form-hint">格式：分 时 日 月 周（例如 0 6 * * * 表示每天东八区 6:00）</span>
        </el-form-item>
    ...
    <el-card v-if="userStore.isAdmin" class="settings-card panel-card" shadow="never">
      <template #header>
        <div class="section-head">
          <div>
            <div class="section-title">注册邀请码</div>
            <div class="section-desc">开启后，除首个管理员外，新用户注册必须输入正确邀请码。</div>
          </div>
      ...
      <el-alert
        ...
        :closable="false"
        class="admin-alert"
        title="这是全站共享的邀请码。保存后立即生效，旧码会立刻失效。"
      />

      <el-form label-width="140px" :model="inviteSettingsForm">
        <el-form-item label="启用邀请码">
          <el-switch v-model="inviteSettingsForm.invite_code_enabled" />
        </el-form-item>
        <el-form-item label="邀请码">
          <el-input
            v-model="inviteSettingsForm.invite_code"
            type="password"
            show-password
            placeholder="4-32 位字母、数字、- 或 _"
            style="max-width: 320px"
          />
          <el-button class="invite-action-btn" @click="generateInviteCode">随机生成</el-button>
          <el-button class="invite-action-btn" @click="copyInviteCode">复制</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="savingInviteSettings" @click="saveInviteSettings">
            保存邀请码配置
          </el-button>
        </el-form-item>
      </el-form>
...
const savingInviteSettings = ref(false)
...
const inviteSettingsForm = reactive({
...
async function loadSettings() {
  ...
  if (userStore.isAdmin) {
    ...
    try {
      const { data: inviteSettings } = await adminApi.getInviteCodeSettings()
      Object.assign(inviteSettingsForm, inviteSettings)
    } catch (e) {
      // 忽略
    }
  }
...
function generateInviteCode() {
  ...
  inviteSettingsForm.invite_code = Array.from(bytes, (item) => alphabet[item % alphabet.length]).join('')
}

async function copyInviteCode() {
  const value = inviteSettingsForm.invite_code.trim()
  if (!value) {
    ElMessage.warning('请先填写邀请码')
    return
  }
  try {
...
async function saveInviteSettings() {
  savingInviteSettings.value = true
  try {
    const { data } = await adminApi.updateInviteCodeSettings({
      invite_code_enabled: inviteSettingsForm.invite_code_enabled,
      invite_code: inviteSettingsForm.invite_code,
    })
    Object.assign(inviteSettingsForm, data)
    ElMessage.success('邀请码配置已保存')
  } finally {
    savingInviteSettings.value = false
  }
}
