<template>
  <div class="asset-page">
    <section class="hero-panel">
      <div class="hero-copy">
        <p class="hero-kicker">Asset Hub</p>
        <h3>角色资产总览</h3>
        <p class="hero-desc">
          把账号下的角色、签到、便笺、抽卡与兑换入口收敛到同一个页面。这里不做复杂养成分析，
          只负责让你快速知道“我有哪些角色”“它们能做什么”“下一步要去哪处理”。
        </p>
      </div>
      <div class="hero-meta">
        <div class="hero-meta-label">当前接入</div>
    ...
    <div class="summary-grid">
      ...
      <div class="summary-card">
        ...
        <div class="summary-note">当前账号体系下已识别的全部游戏角色</div>
      </div>
    ...
    <el-skeleton :loading="loading" animated :rows="10">
      <template #default>
        <el-empty
          ...
          description="当前还没有可展示的角色资产，先完成一次账号导入"
          :image-size="88"
        >
...
const headlineSubtitle = computed(() => {
  if (!overview.value.summary.total_roles) {
    return '先绑定账号并拉取角色信息，角色资产总览才会在这里聚合展示。'
  }
...
function getNotesHint(status: string) {
  if (status === 'available') return '当前具备实时便笺查询前提'
...
.hero-panel {
  ...
  background:
    ...
    radial-gradient(circle at left bottom, rgba(244, 114, 182, 0.18), transparent 30%),
    linear-gradient(135deg, rgba(15, 23, 42, 0.96), rgba(3, 105, 161, 0.9));
  color: #e0f2fe;
