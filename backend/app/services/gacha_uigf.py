...
数据库层继续复用当前扁平表，不在这次任务里引入按 UID 分表或迁移，属于有意为之：
- 现有查询、汇总、去重都建立在 `GachaRecord(account_id, game, record_id)` 这套主线上；
- 若在同一次协议替换里同时改存储模型，会把“交换协议升级”和“历史数据迁移”耦合在一起，回归面明显扩大；
- 代价是导出时只能把同一账号同一游戏的扁平记录重新打包回 UIGF，若未来要严格区分同游戏多 UID，必须另开迁移任务，不能在这里偷偷改行为。
"""
