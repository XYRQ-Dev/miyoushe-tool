...
INVITE_CODE_PATTERN = re.compile(r"^[A-Za-z0-9_-]{4,32}$")
INVITE_CODE_FORMAT_ERROR = "邀请码仅支持 4-32 位字母、数字、连字符或下划线"
INVITE_CODE_REQUIRED_ERROR = "启用邀请码时必须填写邀请码"
INVITE_CODE_MISMATCH_ERROR = "邀请码错误"
...
class SystemSettingsService:
    ...
    async def ensure_required_columns(self) -> None:
        ...
        async with self.db.bind.begin() as conn:
            ...
            if "invite_code_enabled" not in existing_columns:
                await conn.exec_driver_sql(
                    f"ALTER TABLE {SystemSetting.__tablename__} ADD COLUMN invite_code_enabled BOOLEAN DEFAULT 0"
                )
            if "invite_code_encrypted" not in existing_columns:
                await conn.exec_driver_sql(
                    f"ALTER TABLE {SystemSetting.__tablename__} ADD COLUMN invite_code_encrypted VARCHAR(1024)"
                )
    ...
    @classmethod
    def validate_invite_code_format(cls, code: str) -> None:
        if not INVITE_CODE_PATTERN.fullmatch(code):
            raise ValueError(INVITE_CODE_FORMAT_ERROR)

    @staticmethod
    def decrypt_stored_invite_code(config: SystemSetting) -> str:
        encrypted = config.invite_code_encrypted
        if not encrypted:
            return ""
        try:
            return decrypt_text(encrypted)
        except Exception:
            # 密钥漂移或历史脏数据不应让管理员配置页直接 500；当成“尚未配置可复制的明文”
            # 处理，注册校验仍会因为对不上而拒绝，避免误开注册。
            return ""
    ...
    async def is_invite_required(self) -> bool:
        """
        是否要求注册携带正确邀请码。

        首个用户必须能无码完成引导并成为管理员；只有“已有用户 + 已开启 + 已保存邀请码”
        时才对后续注册生效，避免全新安装或旧部署升级后被锁死。
        """
        if await self._count_users() <= 0:
            return False
        config = await self.get_or_create()
        return bool(config.invite_code_enabled and config.invite_code_encrypted)

    async def get_register_options(self) -> RegisterOptionsResponse:
        return RegisterOptionsResponse(invite_required=await self.is_invite_required())

    def invite_code_matches(self, *, config: SystemSetting, raw: str | None) -> bool:
        expected = self.decrypt_stored_invite_code(config)
        provided = self.normalize_invite_code(raw)
        if not expected:
            return False
        return hmac.compare_digest(provided, expected)

    async def assert_register_invite_allowed(self, raw: str | None) -> None:
        if not await self.is_invite_required():
            return
        config = await self.get_or_create()
        if not self.invite_code_matches(config=config, raw=raw):
            raise ValueError(INVITE_CODE_MISMATCH_ERROR)

    async def get_invite_code_settings(self) -> AdminInviteCodeSettingsResponse:
        config = await self.get_or_create()
        return AdminInviteCodeSettingsResponse(
            invite_code_enabled=bool(config.invite_code_enabled),
            invite_code=self.decrypt_stored_invite_code(config),
        )
    ...
    async def update_invite_code_settings(
        ...
        code = self.normalize_invite_code(payload.invite_code)
        if payload.invite_code_enabled and not code:
            raise ValueError(INVITE_CODE_REQUIRED_ERROR)
        if code:
            self.validate_invite_code_format(code)

        config = await self.get_or_create()
        config.invite_code_enabled = payload.invite_code_enabled
        config.invite_code_encrypted = encrypt_text(code) if code else None
        config.updated_at = utc_now_naive()
        self.db.add(config)
        await self.db.commit()
        await self.db.refresh(config)
        return await self.get_invite_code_settings()
