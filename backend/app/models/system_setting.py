...
class SystemSetting(Base):
    ...
    # 注册邀请码是全站共享开关，不是每人一码。密文落库是为了避免库备份或日志把明文带出去；
    # 管理员 GET 仍会解密回传，否则后台无法复制当前码给新用户。
    invite_code_enabled = Column(Boolean, default=False)
    invite_code_encrypted = Column(String(1024), nullable=True)
