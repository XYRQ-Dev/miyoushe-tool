...
- 日志接口与前端展示统一按东八区语义处理
- 仪表盘实时便笺面板，当前基于 `genshin.py` 聚合原神、星铁、绝区零三游实时状态，并支持按角色通过后台功能开关整体禁用
...
- `notes` 作为“仪表盘内实时便笺模块”纳入同一套配置，但不会出现在侧边栏
- 关闭 `notes` 后，首页不再渲染实时便笺面板，前端也不会再请求 `/api/notes/*`
- 即使有人绕过前端直接请求，后端也会对已关闭角色直接返回 `403`
...
### 6. 如果实时便笺上游接口异常，能不能先在后台关掉

可以。

当前 `notes` 已经是独立功能开关，管理员在“菜单与功能开关”里关闭后：

- 首页仪表盘不再渲染实时便笺区块
- 前端不会再发起 `/api/notes/accounts` 与 `/api/notes/summary`
- 后端会直接拒绝对应请求，返回 `403`

当前实时便笺底层已切换为 `genshin.py`，并统一接入原神、星铁、绝区零三游模型；若便笺返回“需验证”，通常表示账号需要先在米游社 App 内重新确认登录或补做一次上游验证。

这意味着在上游接口异常、风控波动、验证链路波动或临时不想暴露该能力时，可以把它当成系统级熔断开关使用，而不是只能等前端报错。
