...
- 默认导出 `v4.2`
- 导入兼容 `v4.0 / v4.1 / v4.2`
- 导入导出都严格按 `account + game + game_uid` 维度隔离
...
- 前端必须先选择“账号 -> 游戏 -> 角色 UID”
- 同一米游社账号下的多个同游戏 UID 必须严格隔离
- 资产页、健康中心等跳转到 `/gacha` 时必须显式透传 `game_uid`
- 不能再依赖“默认第一个角色”或“从链接里自动猜当前 UID”的兜底语义
...
- 原神账号自动导入必须显式绑定当前所选 `game_uid`
- 如果导入链接自带 UID 且与当前所选 UID 不一致，必须拒绝，而不是替用户猜测
